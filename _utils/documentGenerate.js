...
module.exports = {
                    ...
                    async function (err, response) {
                      ...
                      if (templateType == "justMe") {
                        for (var i = 0; emailList.length > i; i++) {
                          emailGenerate.emailGenerate(
                            Document.id,
                            templateObj.userId,
                            emailList[i],
                            templateType,
                            emailList,
                            templateObj.templateMessage,
                            templateObj.templateTitle
                          );
                        }
                      ...
                      if (
                        templateType == "meAndOther" ||
                        templateType == "justOther"
                      ) {
                        const emails = [];
                        if (templateObj.roles && templateObj.roles.length) {
                          if (templateObj.flowType === "serial") {
                            ...
                            templateObj.roles.forEach((role) =>
                              emails.push(role.email)
                            );
                          }
                        }
                        if (templateObj.flowType === "serial") {
  ...
  generateSharedTemplateDocument(
    templateId,
    emailList,
    payload,
    userId,
